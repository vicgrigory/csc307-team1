...
// Database connection removed (handled in backend.js)
...
async function setAsModerator(desiredUsername) {
  ...
  console.log(user);
  console.log(
    `username: ${desiredUsername}, status: ${user.type}, mod?: ${user.type == "moderator"}`,
  );
...
async function getUser(desiredUsername) {
  ...
  if ((await userModel.findOne({ username: desiredUsername })) == undefined) {
...
function addFavorite(desiredUsername, fileID, ...list) {
  ...
  if (!fileID) {
    // Add file check in here. Refer to getFile in its services.
...
*/
async function deleteMyUser(desiredUsername) {
  // how to handle things made by removed users? can we null their data so theyre not technically removed from the db?
...
async function deleteOtherUser(_modId, _deleteId) {
  ...
  if (mod.type == "moderator") {
    ...
    throw new Error(
      "User to be deleted is currently a moderator, demote them first!",
    );
  }
