...
export default function Profile() {
  ...
  const [profilePic, setProfilePic] = useState(
    "https://media.newyorker.com/photos/59095bb86552fa0be682d9d0/master/w_2240,c_limit/Monkey-Selfie.jpg"
  );
    ...
    return (
      <div className="page-container">
        <div className="profile-private-message">
          <img src="https://cdn-icons-png.flaticon.com/512/3064/3064197.png" alt="Locked Icon" className="locked-icon"/>
    ...
    <div className="page-container">
      <div className="profile-header">
        ...
        <div className="profile-picture-wrapper">
          ...
          <label htmlFor="profilePicUpload" className="upload-button">Change Picture</label>
          <input type="file" id="profilePicUpload" accept="image/*" onChange={handleProfilePicChange} style={{ display: "none" }} />
        </div>

        <div className="profile-info">
          <h1 className="profile-username">{username}</h1>
          <div className="profile-settings-tabs">
            <button className="settings-tab" onClick={handleChangePasscode}>Change Passcode</button>
            <button className="settings-tab" onClick={handleChangeEmail}>Change Email</button>
            <button className="settings-tab" onClick={() => setIsPrivate(prev => !prev)}>
      ...
      <div className="profile-bottom-columns">
            ...
            {favoriteMedia.slice(0,5).map(item => (
              <div key={item.id} className="profile-media-card media-horizontal-card">
                ...
                <div className="media-card-content">
                  ...
                  <p className="media-card-meta">{item.type} • {item.author}</p>
                </div>
              </div>
            ))}
          </div>
          {favoriteMedia.length > 5 && <Link to="/favorites" className="view-more-button">View More →</Link>}
        </div>
          ...
          <div className="profile-media-card-wrapper">
            {uploaded.slice(0,5).map(item => (
              <div key={item.id} className="profile-media-card media-horizontal-card">
                ...
                <div className="media-card-content">
                  <h4 className="profile-media-title">{item.title}</h4>
                  <p className="profile-media-meta">{item.type} • {item.author}</p>
                </div>
              </div>
            ))}
          </div>
          {uploaded.length > 5 && <Link to="/uploads" className="view-more-button">View More →</Link>}
        </div>
        ...
        <div className="content-column">
          ...
          {reviews.length > 5 && <Link to="/reviews" className="view-more-button">View More →</Link>}
        </div>
      </div>
    </div>
  );
